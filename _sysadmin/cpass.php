<?php
	$e = mysql_fetch_array(mysql_query("SELECT uUname FROM user WHERE uUname='$_SESSION[medId]'"));
	$ujenis = "Username";
	$uid = $e[uUname];
?>
<div class="row-fluid">
...
<div class="widget-main">
	...
	if (isset($_POST[simpan])){
			...
			$q= mysql_query("UPDATE user SET uPass='$pwd' WHERE uUname='$_POST[kode]'");

			if ($q){
			echo "<script>
			 		notifsukses('Ubah Password','Password Anda Telah Diperbaharui..!!');
			  		setTimeout('window.location.href=\"media.php?page=home\"', 2000)
			      </script>";
			}else{
			echo "<script>
			      notiferror('Ubah Password','Password Anda Gagal Diperbaharui..!!');
