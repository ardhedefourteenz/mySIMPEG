...
<div class="widget-main">
	...
	<form method="POST" enctype="multipart/form-data" class="form-horizontal">
		<div class="control-group">
			...
			<div class="controls">
				<input class="input-xxlarge" type="text" id="kNama" name="kNama" required>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="kProv">Provinsi</label>
			<div class="controls">
				<select class="span3 chosen-select" name="kProv" id="kProv">
					<?php
						$qsp = mysql_query("SELECT * FROM m_provinsi ORDER BY pId ASC");
						while ($s=mysql_fetch_array($qsp)) {
							if ($e['kProv']==$s['pId']){
								echo "<option value='$s[pId]' selected>$s[pNama]</option>";	
							}else{
								echo "<option value='$s[pId]'>$s[pNama]</option>";
							}
	...
	<?php
		if (isset($_POST['simpan'])){
		  	$q = mysql_query("INSERT INTO m_kabkota (kNama,kProv)
		  											VALUES('$_POST[kNama]','$_POST[kProv]')
		                    ");
			if ($q){
			echo "<script>
...
$e = mysql_fetch_array(mysql_query("SELECT * FROM m_kabkota WHERE kId='$_GET[id]'"));
$idx = $e['kId'];
?>
<div class="widget-box">
...
<div class="widget-main">
	...
	<?php
		if (isset($_POST['simpan'])){
			...
			$q = mysql_query("UPDATE m_kabkota SET kNama ='$_POST[kNama]',
																kProv ='$_POST[kProv]',
																onUpdate=NOW()
			                      					WHERE kId = '$_POST[idx]'");
...
?>
	...
	<?php
		if ($_GET['mode']=="hapus"){
			mysql_query("DELETE FROM m_kabkota WHERE kId='$_GET[id]'");
			echo "<script>
	...
	<tbody>
	 <?php
	    $qry = mysql_query("SELECT * FROM m_kabkota");
		while ($d = mysql_fetch_array($qry)){
	      $no++;
	      $prov = getValue("pNama","m_provinsi","pId='$d[kProv]'");
	      echo "
	      <tr>
