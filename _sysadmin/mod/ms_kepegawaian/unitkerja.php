...
<div class="widget-main">
	...
	<form method="POST" enctype="multipart/form-data" class="form-horizontal">
		<div class="control-group">
			...
			<div class="controls">
				<input class="input-xxlarge" type="text" id="uNama" name="uNama" value="<?php echo $e['uNama'];?>" required>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="uInduk">Induk Unit Kerja</label>
			<div class="controls">
				<select class="span3 chosen-select" name="uInduk" id="uInduk">
					<?php
						$qsp = mysql_query("SELECT * FROM m_indukunit ORDER BY uId ASC");
						while ($s=mysql_fetch_array($qsp)) {
							if ($e['uInduk']==$s['uId']){
								echo "<option value='$s[uId]' selected>$s[uNama]</option>";	
							}else{
								echo "<option value='$s[uId]'>$s[uNama]</option>";
							}
		...
		<div class="control-group">
			...
			<div class="controls">
				<input class="input-xxlarge" type="text" id="uKet" name="uKet" value="<?php echo $e['uKet'];?>">
			</div>
		</div>
	...
	<?php
		if (isset($_POST['simpan'])){
		  	$q = mysql_query("INSERT INTO m_unitkerja (uNama,uInduk,uKet)
		  											VALUES('$_POST[uNama]','$_POST[uInduk]','$_POST[uKet]')
		                    ");
			if ($q){
			echo "<script>
...
$e = mysql_fetch_array(mysql_query("SELECT * FROM m_unitkerja WHERE uId='$_GET[id]'"));
$idx = $e['uId'];
?>
<div class="widget-box">
...
<div class="widget-main">
	...
	<?php
		if (isset($_POST['simpan'])){
			...
			$q = mysql_query("UPDATE m_unitkerja SET uNama ='$_POST[uNama]',
																uInduk ='$_POST[uInduk]',
																uKet ='$_POST[uKet]',
																onUpdate=NOW()
			                      					WHERE uId = '$_POST[idx]'");
...
?>
	...
	<?php
		if ($_GET['mode']=="hapus"){
			mysql_query("DELETE FROM m_unitkerja WHERE uId='$_GET[id]'");
			echo "<script>
	...
	<tbody>
	 <?php
	    $qry = mysql_query("SELECT * FROM m_unitkerja");
		while ($d = mysql_fetch_array($qry)){
	      $no++;
	      $induk = getValue("uNama","m_indukunit","uId='$d[uInduk]'");
	      echo "
	      <tr>
	      <td class='center'>$no</td>
	      <td class='left'>$d[uNama]</td>
	      <td class='left'>$induk</td>
	      <td class='center'>
